...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	s += t ? t : '鎻愮ず淇℃伅';
	s += '</em><span><a href="javascript:;" id="fwin_dialog_close" class="flbc" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="鍏抽棴">鍏抽棴</a></span></h3>';
	if(mode == 'info') {
		s += msg ? msg : '';
	} else {
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == '璇疯緭鍏ユ悳绱㈠唴瀹�') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = '璇疯緭鍏ユ悳绱㈠唴瀹�';
	} else {
