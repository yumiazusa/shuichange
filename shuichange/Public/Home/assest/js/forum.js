...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = '鎮ㄧ殑甯栧瓙闀垮害涓嶇鍚堣姹傘€俓n\n褰撳墠闀垮害: ' + mb_strlen(theform.message.value) + ' ' + '瀛楄妭\n绯荤粺闄愬埗: ' + postminchars + ' 鍒� ' + postmaxchars + ' 瀛楄妭';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	if(!quiet && !confirm('姝ゆ搷浣滃皢瑕嗙洊褰撳墠甯栧瓙鍐呭锛岀‘瀹氳鎭㈠鏁版嵁鍚楋紵')) {
		return;
	}
...
function showtime() {
	for(i=0; i<=DTimers.length; i++) {
		if(DItemIDs[i]) {
			...
			if(timer_hour > 0) {
				timestr += timer_hour + '灏忔椂'
			}
			if(timer_minute > 0) {
				timestr += timer_minute + '鍒�'
			}
			if(timer_second > 0) {
				timestr += timer_second + '绉�'
			}
