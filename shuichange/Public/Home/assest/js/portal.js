...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? '鎮ㄧ‘瀹氳鍒犻櫎璇ユ暟鎹悧锛�' : '鎮ㄧ‘瀹氳灞忚斀璇ユ暟鎹悧锛�';
	if(confirm(msg)) {
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(!$('recommendback')) {
			...
			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="cur1">&nbsp;&nbsp;&laquo;杩斿洖</em>';
...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = '灞曞紑璁剧疆椤�';
	} else {
		$('tbody_setting').style.display = '';
		$('a_setting_show').innerHTML = '鏀惰捣璁剧疆椤�';
	}
	doane();
}
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('妯″潡鏍囪瘑涓嶈兘涓虹┖', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('鑷畾涔夊唴瀹归敊璇紝HTML浠ｇ爜锛�'+tag+' 鏍囩涓嶅尮閰�', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('HTML浠ｇ爜锛�'+tag+' 鏍囩涓嶅尮閰�', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('鎮ㄧ‘瀹氳杞崲妯″潡鐨勭被鍨嬩粠 '+ele.options[0].innerHTML+' 鍒� '+ele.options[ele.selectedIndex].innerHTML)) {
...
function strLenCalc(obj, checklen, maxlen) {
	...
	if(curlen <= maxlen) {
		checklen.innerHTML = '宸茶緭鍏� <b>'+(curlen)+'</b> 涓瓧绗�';
		return true;
	} else {
		checklen.innerHTML = '瓒呭嚭 <b style="color:red">'+(curlen - maxlen)+'</b> 涓瓧绗�';
		return false;
	}
}
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
		showDialog('鏍囬闀垮害涓嶆纭�', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
		showDialog('绠€浠嬮暱搴︿笉姝ｇ‘', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}